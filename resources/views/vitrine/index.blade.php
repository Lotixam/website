<!DOCTYPE html>
<html lang="fr">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width,initial-scale=1" />
        <meta name="description" content="LOTIXAM, l'investisseur immobilier professionnel si particulier, en Charente-Maritime, Nord-Gironde et Charente." />
        <meta name="keywords" content="immobilier, investisseur, investissement locatif, professionnel, Lotixam, maison, maison neuve, maison individuelle, appartement, immeuble, agence, constructeur, construction, vente, terrain, terrain à bâtir, terrain constructible, charente maritime, gironde, charente, marchand de biens">
        
        <title>LOTIXAM</title>
        <link href="/img/logo.png" rel="icon" type="image/x-icon">
    
        <link href="/css/common.css" rel="stylesheet">
        <link href="/css/welcome.css" rel="stylesheet">
        <link href="/css/banniere/banniere.css" rel="stylesheet">
        <link href="/css/foot.css" rel="stylesheet">
        <script src="/script/welcome.js"></script>

    </head>
    <body id="page">
        <script src="/script/welcome.js"></script>
        <!--Bannière-->
        <div id="banniere">

            <div class="sign ">
                @include('vitrine.partials.login-nav-link', ['guestLabel' => "Se connecter / S'inscrire"])
            </div>

            <div id="logo">
                <img src="/img/logo.png" width="170" alt="Logo du site lotixam">
                <div id="devanture">
                    <img src="/img/lotixam.png" width="150" alt="Davanture texte du site lotixam">
                    <label id="sentence">L'INVESTISSEUR IMMOBILIER PROFESSIONNEL SI PARTICULIER</label>
                </div>
            </div>
            <!-- Type de lien en mode Bureau -->
            <div id="liens">
                <div> 
                    <a href="/" class="sous-liens">
                        Accueil
                    </a>
                </div>
                <div>
                    <a href="/qui-sommes-nous" class="sous-liens">
                        Qui sommes nous
                    </a>
                </div>
                <div>
                    <a href="/nous-achetons" class="sous-liens">
                        Nous achetons
                    </a>
                </div>
                <div>
                    <a href="/nous-vendons" class="sous-liens">
                        Nous vendons
                    </a>
                </div>
                <div>
                    <a href="/investisseurs" class="sous-liens">
                        Investisseurs
                    </a>
                </div class="sous-liens">
                <div class="sous-liens">
                    <a href="/contact?prev=index&amp;button=false" class="sous-liens">
                        Contact
                    </a>
                </div>
            </div>
            
            <!-- Structure du menu hamburger -->
            <div class="menu-toggle">
                <input type="checkbox" id="toggle">
                <label for="toggle">&#9776;</label> <!-- icône du menu hamburger -->
                <ul class="menu">
                    <li class="liens"><a href="/">Accueil</a></li>
                    <li class="liens"><a href="/qui-sommes-nous">Qui sommes nous</a></li>
                    <li class="liens"><a href="/nous-achetons">Nous achetons</a></li>
                    <li class="liens"><a href="/nous-vendons">Nous vendons</a></li>
                    <li class="liens"><a href="/investisseurs">Investisseurs</a></li>
                    <li class="liens"><a href="/contact?prev=index&amp;button=false">Contact</a></li>
                    <li class="liens">@include('vitrine.partials.login-nav-link', ['guestLabel' => 'Mon compte'])</li>
                </ul>
            </div>
        </div>

        <!--Corps de la page-->

        <!-- Image de Cannelle -->
        <div id="image">
            <div class="image_text">
                <div>
                    <h1>Vous cherchez un partenaire fiable ?</h1>
                </div>
            </div>
        </div>

        <!-- - Corps principal -->
        <div class="content" style="margin: 1em;">

            <!-- Bienvenue -->
            <div id="paragraphe1" class="para">
                <div class="elem1"></div>
                <div class="elem2"></div>
                <h1 class="first-h1">
                    Bienvenue chez <b>LOTIXAM</b>, votre partenaire immobilier de confiance, opérant avec passion dans les régions
                    pittoresques de la Charente-Maritime, du Nord-Gironde et de la Charente.
                    <br><br>
                    Chez <b>LOTIXAM</b>, nous ne nous contentons pas de vendre des biens, nous créons des
                    opportunités pour donner vie à vos projets immobiliers.
                </h1>
            </div>

            @if(isset($publicMetrics) && $publicMetrics->isNotEmpty())
                <section class="public-metrics">
                    <h2>Nos chiffres clés</h2>
                    <div class="public-metrics-grid">
                        @foreach($publicMetrics as $metric)
                            <article class="public-metric-card">
                                <div class="public-metric-value">{{ $metric['value'] }}{{ $metric['suffix'] }}</div>
                                <p class="public-metric-label">{{ $metric['label'] }}</p>
                            </article>
                        @endforeach
                    </div>
                </section>
            @endif

            <!-- Tableau  /!\-->
            <table>
                <tr>
                    <td>
                        <div class="h2-icons-zone">
                            <div class="h2-icons">
                                <img src="/img/zone-avec-epingles.png" width="50" alt="icone zone avec épingle Lotixam">
                            </div>
                        </div>
                        <h2>Pourquoi la Charente-Maritime ?</h2>
                        <div id="paragraphe2" class="para">
                            <div class="elem1"></div>
                            <div class="elem2">
                                <p>
                                    La Charente-Maritime est la première destination touristique de France, attirant près de trois millions de
                                    visiteurs chaque année. Investir ici, c’est choisir un cadre de vie idyllique tout en
                                    bénéficiant
                                    d’une valeur immobilière en constante appréciation.
                                </p>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div class="h2-icons-zone">
                            <div class="h2-icons">
                                <img src="/img/identification.png" width="50" alt="icone loupe sur empreinte Lotixam">
                            </div>
                        </div>
                        <h2>Notre Approche Unique</h2>
                        <div id="paragraphe3" class="para">
                            <div class="elem1"></div>
                            <div class="elem2">
                                <p>
                                    Chez <b>LOTIXAM</b>, notre force réside dans notre capacité à accompagner nos clients du début à la fin de leur
                                    projet
                                    immobilier. Nous simplifions le processus complexe de l’immobilier en vous offrant un service d'accompagnement complet.
                                </p>
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td>
                        <div class="h2-icons-zone">
                            <div class="h2-icons">
                                <img src="/img/renover.png" width="50" alt="icone de rénovation de maison Lotixam">
                            </div>
                        </div>
                        <h2>Rénovation et Transformation</h2>
                        <div id="paragraphe4" class="para">
                            <div class="elem1"></div>
                            <div class="elem2">
                                <p>
                                    En plus de nos services complets de construction et de division foncière, nous transformons les vieux murs en
                                    espaces de vie
                                    modernes
                                    et fonctionnels, augmentant ainsi la valeur de nos investissements afin d'être au plus près de vos
                                    attentes.
                                </p>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div class="h2-icons-zone">
                            <div class="h2-icons">
                                <img src="/img/euro.png" width="50" alt="icone d'amats de pièces Lotixam">
                            </div>
                        </div>
                        <h2>Investissements Partagés</h2>
                        <div id="paragraphe5" class="para">
                            <div class="elem1"></div>
                            <div class="elem2">
                                <p>
                                    Chez <b>LOTIXAM</b>, nous ouvrons la porte aux opportunités d'investissement collaboratif, offrant à la fois
                                    aux
                                    professionnels
                                    et aux particuliers la chance de participer à des projets immobiliers à fort potentiel de rentabilité. 
                                </p>
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td>
                        <div class="h2-icons-zone">
                            <div class="h2-icons">
                                <img src="/img/achat.png" width="50" alt="icone donner de l'artgent main à main Lotixam">
                            </div>
                        </div>
                        <h2>Nous achetons</h2>
                        <div id="paragraphe7" class="para">
                            <div class="elem1"></div>
                            <div class="elem2 achat">
                                <p>
                                    <ul>
                                        <li>
                                            Terrains constructibles
                                        </li>
                                        <li>
                                            Maisons individuelles
                                        </li>
                                        <li>
                                            Immeubles
                                        </li>
                                        <li>
                                            Ensembles immobiliers
                                        </li>
                                        <li>
                                            Locaux commerciaux
                                        </li>
                                        <li>
                                            Bureaux
                                        </li>
                                        <br>
                                        Offre d'achat rapide sans conditions suspensives.
                                    </ul>
                                </p>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div class="h2-icons-zone">
                            <div class="h2-icons">
                                <img src="/img/gens.png" width="50" alt="icone d'échange mutuel Lotixam">
                            </div>
                        </div>
                        <h2>Nous vendons</h2>
                        <div id="paragraphe8" class="para">
                            <div class="elem1"></div>
                            <div class="elem2">
                                <p>
                                    Chaque propriété que nous mettons en vente offre un caractère unique et des espaces de vie modernes, conçus pour offrir
                                    confort et fonctionnalité à leurs occupants.
                                </p>
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <div class="h2-icons-zone">
                            <div class="h2-icons">
                                <img src="/img/engagement.png" width="50" alt="icone d'engagement Lotixam">
                            </div>
                        </div>
                        <h2>Engagement fort sur la Satisfaction Client</h2>
                        <div id="paragraphe6" class="para">
                            <div class="elem1"></div>
                            <div class="elem2 last-child">
                                <p>
                                    Votre satisfaction est au cœur de notre démarche. Chez <b>LOTIXAM</b>, chaque client est unique, et nous
                                    nous
                                    engageons à
                                    fournir une expérience personnalisée et attentive. Nous comprenons l'importance de l'investissement
                                    immobilier et
                                    nous
                                    nous efforçons de dépasser vos attentes à chaque interaction.
                                </p>
                                <br>
                                <p>
                                    Nous vous invitons à découvrir comment <b>LOTIXAM</b> peut vous aider à réaliser votre projet immobilier en
                                    Charente-Maritime, Nord-Gironde et Charente.
                                </p>
                            </div>
                            <div class="contact" id="contact-welcome">
                                <button class="contact-button" id="contact-button-index-1" onclick="window.location.href = '/contact?prev=index&amp;button=true';">CONTACTEZ-NOUS</button>
                            </div>
                        </div>
                    </td>
                </tr>
            </table>
        </div> <!-- Fin des paragraphes -->
        <div class="youtube">
            <img src="/img/diagLotixam.png" alt="Image explicatif de toute la procédure sans texte du projet à la fin de la réalisation">

            <!-- <iframe class="youtube-video" width="1020" height="630" src="https://www.youtube.com/embed/0gV9y0RF2Uk?si=D0ktrHVkjhMwHwHv"
                title="YouTube video player" frameborder="0"
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                referrerpolicy="strict-origin-when-cross-origin" allowfullscreen>
            </iframe> -->
        </div>
    </body>
    @include('vitrine.partials.footer')
</html>